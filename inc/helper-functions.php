...
function kasutan_mail_secretariat($atts) {
	extract( shortcode_atts( array(    
		'label' => ' '
		), $atts) );
	...
	if(function_exists('get_field')) {
		$email=sanitize_email(get_field('pizza_email','option'));
	}
	if($email) {
		return sprintf('<a href="mailto:%s">%s</a>',antispambot($email),$label);
	}
	return '';
}
		
add_shortcode( 'email-secretariat', 'kasutan_mail_secretariat' );
/**
...
/*Préparer un CSV avec la première image comme featured image*/ 
//add_shortcode('featured-image','kasutan_auto_featured_image');
function kasutan_auto_featured_image($atts) {
	ob_start();
	...
	if($posts) {
		printf('<p>%s posts</p>',count($posts));
		echo '<table id="recup-img"><tbody>';
		foreach($posts as $post) {
			$post_id=$post->ID;
			$content=apply_filters('post_content',$post->post_content);
		echo '<tr>';
			printf('<td><a href="/wp-admin/post.php?post=%s&action=edit" target="_blank">%s</a></td>',$post_id,$post_id);
			printf('<td>%s</td>',$post_id);
			printf('<td class="js-url-img"></td>');
			printf('<td>%s</td>',get_the_title($post));
			printf('<td class="js-content">%s</td>',$content);
			echo '</tr>';
		}
		echo '</tbody></table>';
